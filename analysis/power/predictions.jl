using CSV, DataFrames, IdentityByDescentDispersal, Turing, StatsBase, Random, CodecZlib, DelimitedFiles
using QuadGK # Using Gaussian quadrature rules

mle_short = CSV.read("short_mle.csv", DataFrame)
mle_long = CSV.read("long_mle.csv", DataFrame)

# Constants
contig_lengths = vec(readdlm(GzipDecompressorStream(open("../../data/input_contig_lengths.txt.gz")), ',', Float64, '\n'))
De(t, params) = params[1] * t^-params[2] # Power-law
grid_times = 0:1:600
ibd_bins = [
    (1.0 / 100, 2.5 / 100, "1.0-2.5 cM"),
    (2.5 / 100, 5.0 / 100, "2.5-5.0 cM"),
]
n_sample = 1000
grid_r1 = [2.5, 20, 180] # Geographic distances in km
grid_r2 = [100, 1000, 2000] # Geographic distances in km

# Extract MLE parameters
mle_short_D = mle_short[1, "Coef."]
mle_short_beta = mle_short[2, "Coef."]
mle_short_sigma = mle_short[3, "Coef."]
mle_long_D = mle_long[1, "Coef."]
mle_long_beta = mle_long[2, "Coef."]
mle_long_sigma = mle_long[3, "Coef."]

# Create tidy dataframe with predictions for all combinations
predictions = DataFrame(
    time=Float64[],
    distance_km=Float64[],
    scale=String[],
    bin=String[],
    density=Float64[]
)

for r in grid_r1
    for (left, right, bin_label) in ibd_bins
        Ls = range(left, right, n_sample)
        densities = [age_density_ibd_blocks_custom(
            grid_times, r, De,
            [mle_short_D, mle_short_beta], mle_short_sigma,
            L_val, contig_lengths
        ) for L_val in Ls]
        avg_dens = mean(densities)
        for (i, t) in enumerate(grid_times)
            push!(predictions, (time=t, distance_km=r, scale="short", bin=bin_label, density=avg_dens[i]))
        end
    end
end

for r in grid_r2
    for (left, right, bin_label) in ibd_bins
        Ls = range(left, right, n_sample)
        densities = [age_density_ibd_blocks_custom(
            grid_times, r, De,
            [mle_long_D, mle_long_beta], mle_long_sigma,
            L_val, contig_lengths
        ) for L_val in Ls]
        avg_dens = mean(densities)
        for (i, t) in enumerate(grid_times)
            push!(predictions, (time=t, distance_km=r, scale="long", bin=bin_label, density=avg_dens[i]))
        end
    end
end

# Save the tidy predictions dataframe
CSV.write("predictions_age_ibd.csv", predictions)

# Absolute goodness of fit
predict_short(distance, left, right) = quadgk(x -> expected_ibd_blocks_power_density(distance, mle_short_D, mle_short_beta, mle_short_sigma, x, contig_lengths), left, right)[1]
predict_long(distance, left, right) = quadgk(x -> expected_ibd_blocks_power_density(distance, mle_long_D, mle_long_beta, mle_long_sigma, x, contig_lengths), left, right)[1]

# Short scale predictions for each row
short_agg = CSV.read(GzipDecompressorStream(open("../short_agg_10km.csv.gz")), DataFrame)
short_agg = short_agg[short_agg.distance_bin.>0, :]
short_agg.prediction = [predict_short(row.distance_bin, row.IBD_LEFT, row.IBD_RIGHT) for row in eachrow(short_agg)]
CSV.write("short_predictions.csv", short_agg)

# Long scale predictions for each row
long_agg = CSV.read(GzipDecompressorStream(open("../long_agg_100km.csv.gz")), DataFrame)
long_agg = long_agg[long_agg.distance_bin.>0, :]
long_agg.prediction = [predict_long(row.distance_bin, row.IBD_LEFT, row.IBD_RIGHT) for row in eachrow(long_agg)]
CSV.write("long_predictions.csv", long_agg)
