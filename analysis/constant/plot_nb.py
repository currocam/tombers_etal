...
def _(data, plt, sns):
    ...
    sns.lineplot(
        data = data[data.scale=="short"], x = "time", y ="density", hue ="distance_km",
        ax=ax1, palette = ["C0", "C1", "C2"],
    )
    ax1.legend(title="")
    plt.ylabel("Density of expected IBD\n  blocks shared per pair")
...
def _(data, plt, sns):
    ...
    sns.lineplot(
        data = data[data.scale=="long"], x = "time", y ="density", hue ="distance_km",
        ax=ax2, palette = ["C3", "C4", "C5"],
    )
    ax2.legend(title="")
    plt.ylabel("Density of expected IBD\n  blocks shared per pair")
    plt.xlabel("Time (generations ago)")
...
def _(pd):
    ...
    data2["BIN_INDEX"] = data2["BIN_INDEX"].map({1 : "0.4-1.0 cM", 2 : "1.0-2.5 cM", 3 : "2.5-4.0 cM"})
    data2
    return (data2,)
...
def _(pd):
    ...
    data3["BIN_INDEX"] = data3["BIN_INDEX"].map({1 : "0.4-1.0 cM", 2 : "1.0-2.5 cM", 3 : "2.5-4.0 cM"})
    data3
    return (data3,)
...
def _(data2, np):
    ...
    num_draws = 10_000 
    pred_conf = np.array([
        scipy.stats.poisson(rate).rvs((n, num_draws), rng).mean(axis=0) for rate, n in zip(data2["prediction"], data2["n"])
    ])
    lower_pred, upper_pred = np.quantile(pred_conf, [0.025, 0.975], axis=1)
    data2["lower_pred"] = lower_pred
    data2["upper_pred"] = upper_pred
    return num_draws, rng, scipy
...
def _(data2, plt, sns):
    ...
    for i, label in enumerate([
        ...
        ax3.errorbar(
            subset["distance_bin"], subset["mean"],
    ...
    ax3.legend([])
    fig3.legend(
        title="", loc='outside lower center', ncol=3,
...
@app.cell
def _(data3, np, num_draws, rng, scipy):
    n2 = data3.shape[0]
    pred_conf2 = np.array([
        scipy.stats.poisson(rate).rvs((n, num_draws), rng).mean(axis=0) for rate, n in zip(data3["prediction"], data3["n"])
    ])
...
def _(data3, plt, sns):
    ...
    for j, label2 in enumerate([
        ...
        g3 = sns.lineplot(
            data=subset2, x = "distance_bin",
            y = "prediction",ax=ax4,
            color = f"C{j}", label = label2
        )
        ax4.fill_between(
        ...
        ax4.errorbar(
            subset2["distance_bin"], subset2["mean"],
